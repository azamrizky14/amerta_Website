...
onMounted( async () => {
  const data = await adminTeknis_GetDataByDomainAndStatusAndType('N', 'MT')
          ...
          <tr v-for="(item, index) in dataTable" :key="index" class="border">
            ...
            <td class="py-1 px-4">
              <div class="flex items-center space-x-3.5 d-flex justify-center">              
                <router-link class="hover:text-primary" :to="'/modules/work-order/evident/mt/detail/'+item._id">
                  <svg
                    class="fill-current"
                    width="18"
                    height="18"
                ...
                <router-link class="hover:text-primary" :to="'/modules/work-order/evident/mt/edit/'+item._id">
